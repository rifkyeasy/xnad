...
users.put('/:walletAddress/strategy', zValidator('json', UpdateStrategySchema), async (c) => {
  const walletAddress = c.req.param('walletAddress');
  const input = c.req.valid('json');
...
users.post('/:walletAddress/analyze-x', zValidator('json', AnalyzeXSchema), async (c) => {
  const walletAddress = c.req.param('walletAddress');
