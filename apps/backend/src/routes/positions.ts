...
positions.get('/:walletAddress/summary', async (c) => {
  ...
  if (USE_MOCK_DATA) {
    ...
    const totalValue = mockPositions.reduce((sum, p) => sum + parseFloat(p.currentValue), 0);
    const totalCostBasis = mockPositions.reduce((sum, p) => sum + parseFloat(p.costBasis), 0);
