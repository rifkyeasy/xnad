/**
 * Create SHRAMP Token on nad.fun
 */
...
async function main() {
  ...
  console.log('Creating SHRAMP Token on nad.fun (Monad Testnet)');
  console.log('='.repeat(60));
  ...
  const tokenParams = {
    name: 'Shramp',
    symbol: 'SHRAMP',
    description: 'Shramp - The shrimpy meme token on Monad. Small but mighty, riding the waves of the blockchain ocean.',
    imagePath: './shramp.png',
  ...
  try {
    ...
    console.log(`shramp: '${result.tokenAddress}',`);
  } catch (error) {
